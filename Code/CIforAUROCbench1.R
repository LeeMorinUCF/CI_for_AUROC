...
bipower_gen <- function(n_x, n_y, 
  ...
  # Initialize data frame with outcomes and weights. 
  bipower_df <- data.frame(outcomes = c(rep(FALSE, n_x),rep(TRUE, n_y)),
                           weights = c(rep(1/n_x, n_x),rep(1/n_y, n_y)))
  
  
  # Depends on discrete or continuous specification of the power law. 
  if (disc_or_cts == 'cts') {
    
    # Realizations are generated by inverting the power law CDF. 
    bipower_df[, 'scores'] <- c(rplcts(n = n_x, xmin = x_min, alpha = alpha_x), 
                                rplcts(n = n_y, xmin = y_min, alpha = gamma_y))
    
  } else  { # (disc_or_cts == 'disc')
    
    # Realizations are generated using the rpldis function. 
    bipower_df[, 'scores'] <- c(rpldis(n = n_x, xmin = x_min, alpha = alpha_x), 
...
#--------------------------------------------------------------------------------
# Generate realizations from a power law model
#--------------------------------------------------------------------------------

rplcts <- function(n, xmin, alpha) {
  
  # First draw a realization of the uniform distribution. 
  x_pl_cts <- runif(n)
  
  # Next, transform it through the inverse of the CDF.
  x_pl_cts <- xmin*(1 - x_pl_cts)^(-1/(alpha - 1))
...
# Test:
# rplcts(n = 10, xmin = 2, alpha = 2)
# rplcts(n = 20, xmin = 2, alpha = 1.2)
# summary(rplcts(n = 100, xmin = 2, alpha = 1.1))
